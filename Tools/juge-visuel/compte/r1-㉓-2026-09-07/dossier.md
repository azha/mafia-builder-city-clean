# Dossier du juge visuel — ㉓ La vitrine (IAP Shop) — r1-㉓ — 2026-09-07
...
- **Nom** : La vitrine (IAP Shop) (㉓, canon `screen_c2`) — contrôleur `ShopScreenController`
- **Ce qu'on vient y faire** : le catalogue, le solde, l'achat, les entitlements, la restauration (front.md « Montre »)
- **Chemin joueur emprunté par la capture** : Plus → LA VITRINE
- **États capturés** : PREMIER TOUR (r1) — aucune planche jugée avant. 2 capture(s), 2 campagne(s) : `03efb90` 06/09 14:56 · `cf99604` 06/09 15:31. Planches = blobs commités sur `main` du client (`fd0e21e` est le tip de la base de preuve du 06/09 soir) ; aucun correctif d'écran antérieur ⇒ pas de descendance à vérifier, pas de tour précédent.

## Référence (fait autorité : l'IMAGE)

| fichier (dans ce dossier) | rôle | taille px | facteur | largeur CSS ↔ largeur écran |
|---|---|---|---|---|
| `reference-㉓-1080x2102.png` | rendu du cadre nominal (`ecrans-brennar-6.html` #98) | 1080×2102 | ×3,6 | 300 CSS = 1080 px |
| `etats/boutique-canon.png` | canon de la série 2 (catalogue) | 900×1752 | ×3,0 | 300 CSS = 900 px |
| `etats/boutique-extra-possede.png` | canon de la série 2 (un extra possédé) | 900×1752 | ×3,0 | 300 CSS = 900 px |
| `etats/vide-maquette-vitrine.png` | maquette d'ÉTAT VIDE de l'atelier (`4e6697f`, `da/portraits-lieutenants`) — asset NON monté dans le client : juge le SENS, pas le pixel | 1024×1024 | inconnu | largeur 1024 px — échelle à établir par le juge (largeur / 300 CSS si c'est un .tel) |
...
- **Source HTML/CSS** (aide de lecture, ne prime JAMAIS sur l'image) : `/home/erutheone/project/atelier3d-mafia/ecrans-brennar-6.html` (atelier `9fd7b6f` ;
  ...
  - #98 (l.5478) — La vitrine — les extras  ⇐ **cadre NOMINAL, rendu en référence**
  - #99 (l.5481) — La caisse — les jetons
  - #100 (l.5484) — Zéro jeton — l'état terminal
...
|---|---|---|---|---|
| `capture-1080x2400.png` | 1080×2400 — PRINCIPALE | sous chrome, par la suite de l'écran (`LaVitrineCapturePlayModeTests`) — campagne `cf99604` (06/09 15:31) | 06/09 15:31 (`cf99604`) | `la_vitrine_1080x2400.png — sha256 409de4c2…` |
| `capture-planche-1080x2400.png` | 1080×2400 | surimpression sous chrome (test de planche) — autre campagne — campagne `03efb90` (06/09 14:56) | 06/09 14:56 (`03efb90`) | `planche_la_vitrine_1080x2400.png — sha256 dcc13c25…` |

- Client au moment des captures : `03efb90` (`main` du client, 06/09 14:56) / `cf99604` (`main` du client, 06/09 15:31) — une capture par campagne, voir la table ; ce dossier est préparé sur `28b9da5`. Une capture est une mesure DATÉE.
...
- **Identité par campagne — DÉCLARÉE par le CORPS DE COMMIT (aucun journal joint dans l'arbre ; `journal-declare.txt` transcrit), jamais relue** :
  ...
  - `cf99604` (06/09 15:31) : « avec la paire » + empreinte du compte gelé **72 118** · 17 · 3 · 2 · 7 · 1 — DÉCLARÉ (`demo_capture`).
...
- **Contenu déclaré par les corps de commit** : le corps de commit `1d3d412` signalait sur `la_vitrine` : « — jetons », un tiret à la place de la valeur, corps vide ; `fd0e21e` dit l'avoir RESTAURÉE telle quelle (non recapturée) — la planche `cf99604` est donc celle de 15:31. Vérifie le tiret ; un tiret là où la maquette met un nombre est un écart de SENS (le joueur ne sait pas combien il a).
...
⚠️ Premier tour : **aucune table d'écarts assumés** n'a été inventoriée pour cet écran — tu classes toi-même (ASSUMÉ n'existe donc que pour les règles de doctrine ci-dessous). Un rapport juge-données mode maquette existe pour cet écran (`Tools/juge-donnees/compte/maquette-2026-08-25/rapport.md`) : il n'est PAS fourni (tu ne lis aucun rapport) ; ses défauts de données n'ont pas été transcrits ici — si une valeur affichée te paraît sans source, classe-la « non vérifié », pas écart. Rappels : phase « — » hors district = état VOULU ; ronds du dock vides = arbitrage user connu ; CHALEUR/HEAT ou anglais dans la RÉFÉRENCE = maquette en retard (blender), jamais un écart ; une clé i18n BRUTE ou un libellé de repli visible sur la CAPTURE = écart de SENS ; cadre de style (sombre, napolitain, mafieux, fin 80s–début 90s) : divergence de DIRECTION = ARBITRAGE, jeton/matière/asset absent = écart.
