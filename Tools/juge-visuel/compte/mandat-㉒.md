# Mandat pré-rempli — ㉒ Player Profile — dossier `compte`
...
## L'écran
- **Nom** : Player Profile (㉒, canon `screen_c1`) — contrôleur `ProfileScreenController`
- **Ce qu'on vient y faire** (pré-rempli, front.md « Montre ») : callsign, cash, compte (email masqué, `lifecycle_state`), sauvegardes, historique IAP,
- **Chemin joueur pour y arriver** : Plus → VOTRE PROFIL
- **Routes lues dans le contrôleur** : aucune chaîne `/v1/` dans le dossier du contrôleur (les routes vivent dans un client partagé ailleurs — voir juge-donnees) (`Assets/Scripts/Account/Profile/*.cs`)
- **État `front.md`** (en-tête) : ★ *déclencheur de §S10*

## Référence (fait autorité : l'IMAGE)
| fichier | rôle | taille px | facteur | largeur CSS ↔ largeur Unity |
|---|---|---|---|---|
| `compte/reference-㉒-1080x2102.png` | cadre nominal `ecrans-brennar-6.html` #95 rendu | 1080×2102 | ×3.6 | 300 CSS = 1080 px |
...
- **Cadres de la maquette** : `ecrans-brennar-6.html` 95, 96, 97 — atelier `3c02f72`. Cadres d'ÉTATS : les autres numéros du groupe.
- **Attribution cadre ↔ écran** : déduite. le contrôleur dit « cadres 45-47 » (autre numérotation) ; par le titre : 95-97 « Le compte » ; canon compte/profil-canon.png ; ⚠️ sa planche s'appelle planche_le_coffre
