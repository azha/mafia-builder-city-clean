# Dossier du juge visuel — ㊳ Le journal & la rue (« ce qui se dit ce matin ») — r1 — 2026-09-07
...
- **Nom** : Le journal & la rue (« ce qui se dit ce matin ») (㊳, canon `écran neuf, sans id canon`) — contrôleur `JournalScreenController`
...
- **Chemin joueur emprunté par la capture** : Plus → LE JOURNAL & LA RUE
- **États capturés** : PREMIER TOUR (r1) — aucune planche jugée avant. 3 capture(s), 2 campagne(s) : `31d8e43` 06/09 10:50 · `fd0e21e` 06/09 20:53. Planches = blobs commités sur `main` du client (`fd0e21e` est le tip de la base de preuve du 06/09 soir) ; aucun correctif d'écran antérieur ⇒ pas de descendance à vérifier, pas de tour précédent.

## Référence (fait autorité : l'IMAGE)

| fichier (dans ce dossier) | rôle | taille px | facteur | largeur CSS ↔ largeur écran |
|---|---|---|---|---|
| `reference-1080x2102.png` | rendu du cadre nominal (`ecrans-brennar-6.html` #125) | 1080×2102 | ×3,6 | 300 CSS = 1080 px |
| `etats/vide-maquette-journal.png` | maquette d'ÉTAT VIDE de l'atelier (`4e6697f`, `da/portraits-lieutenants`) — asset NON monté dans le client : juge le SENS, pas le pixel | 1024×1024 | inconnu | largeur 1024 px — échelle à établir par le juge (largeur / 300 CSS si c'est un .tel) |
...
- **Source HTML/CSS** (aide de lecture, ne prime JAMAIS sur l'image) : `/home/erutheone/project/atelier3d-mafia/ecrans-brennar-6.html` (atelier `9fd7b6f` ;
  ...
  - #125 (l.6119) — Le journal — ce qui se dit ce matin  ⇐ **cadre NOMINAL, rendu en référence**
  - #126 (l.6122) — Ce qui arrive à la ville
  - #127 (l.6125) — La rivière ne se retirera pas
  - #128 (l.6128) — Ce que vous avez compris
  - #129 (l.6131) — Rien ce matin
  - #130 (l.6134) — Ce qui manque encore
...
|---|---|---|---|---|
| `capture-1080x2400.png` | 1080×2400 — PRINCIPALE | sous chrome via Plus (`VuePrincipaleCapturePlayModeTests`) — identité MUETTE — campagne `31d8e43` (06/09 10:50) | 06/09 10:50 (`31d8e43`) | `screen_c1_journal_sous_chrome_1080x2400.png — sha256 616f11dc…` |
| `capture-ecran-seul-1080x2400.png` | 1080×2400 | écran SEUL sans chrome (`JournalScreenPlayModeTests`) — autre campagne — campagne `fd0e21e` (06/09 20:53) | 06/09 20:53 (`fd0e21e`) | `screen_c1_1080x2400.png — sha256 b0c65031…` |
| `capture-ecran-seul-1080x1920.png` | 1080×1920 | idem, 16:9 — campagne `fd0e21e` (06/09 20:53) | 06/09 20:53 (`fd0e21e`) | `screen_c1_1080x1920.png — sha256 b25eca1a…` |

- Client au moment des captures : `31d8e43` (`main` du client, 06/09 10:50) / `fd0e21e` (`main` du client, 06/09 20:53) — une capture par campagne, voir la table ; ce dossier est préparé sur `28b9da5`. Une capture est une mesure DATÉE.
- Compte photographié : compte de CAPTURE `demo_capture@example.test` (gelé, puis dérivant entre campagnes — voir la note) **sauf `31d8e43` : identité MUETTE** — identité DÉCLARÉE par corps de commit, non relue ; valeurs non comparables à la référence ni aux corps ⇒ non vérifié ; FORME jugée.
...
- **Identité par campagne — DÉCLARÉE par le CORPS DE COMMIT (aucun journal joint dans l'arbre ; `journal-declare.txt` transcrit), jamais relue** :
  - `31d8e43` (06/09 10:50) : ⚠️ **AUCUNE déclaration d'identité ni empreinte** dans le corps de commit ; il dit « le compte de démo a été re-semé aujourd'hui (TD-642), le CONTENU a bougé » — identité **MUETTE** ⇒ valeurs « non vérifié », forme jugée.
  - `fd0e21e` (06/09 20:53) : « recapture sur demo_capture après le merge » : **72 155** · 20 bâtiments · 3 lt · 2 planques · 8 cartes — DÉCLARÉ nommément.
- ⚠️ **Le compte « gelé » a DÉRIVÉ entre les campagnes** (empreintes lues dans les corps de commit par f2) : 06:56 → 72 013 min / 17 bât. / 6 cartes · 14:56 et 15:31 → 72 118 / 17 / 7 · 20:53 → 72 155 / 20 / 8. Chaque campagne est stable EN SON SEIN ; **deux planches de campagnes différentes photographient DEUX MONDES** : un écart de CONTENU entre elles n'est pas un défaut d'écran — ne conclus jamais à une régression entre deux campagnes. La comparaison se fait planche ↔ maquette, jamais planche ↔ planche.
- **Contenu déclaré par les corps de commit** : le corps de commit `03efb90` signalait sur `screen_c1_1080x2400` : une clé i18n BRUTE au joueur (`press.outlet.free_weekly.name`) — la planche fournie ici pour cette famille est celle de `fd0e21e` (20:53, recapturée) ; une clé brute visible est un écart de SENS (BLOQUANT si c'est un titre).
...
```
(a) deux résolutions 1920+2400            OUI
...
(i) identité                               DÉCLARÉE par corps de commit, par campagne (ci-dessus) ; MUETTE pour `31d8e43`
```
