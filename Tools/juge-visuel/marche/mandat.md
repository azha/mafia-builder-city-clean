# Mandat pré-rempli — ㉑ Substance Market — dossier `marche`
...
## L'écran
- **Nom** : Substance Market (㉑, canon `screen_b1`) — contrôleur `Market (non monté)`
- **Ce qu'on vient y faire** (pré-rempli, front.md « Montre ») : la table des lanes, les prix, la confiance, le passage d'ordre.
- **Chemin joueur pour y arriver** : —
...
- **État `front.md`** (en-tête) : · « Marché — le patron table »  ★ *déclencheur de §S9*

## Référence (fait autorité : l'IMAGE)
| fichier | rôle | taille px | facteur | largeur CSS ↔ largeur Unity |
|---|---|---|---|---|
| `marche/reference-1080x2102.png` | cadre nominal `ecrans-brennar-6.html` #101 rendu | 1080×2102 | ×3.6 | 300 CSS = 1080 px |
| `marche/ecran-canon.png` | canon existant (900×1752, ×3) | — | ×3 | 300 CSS = 900 px |
- **Cadres de la maquette** : `ecrans-brennar-6.html` 101, 102, 103, 104, 105, 106 — atelier `3c02f72`. Cadres d'ÉTATS : les autres numéros du groupe.
- **Attribution cadre ↔ écran** : déduite. cadres 101-106 (le tableau) ; écran bloqué (front.md) ; la colonne des quartiers tronque les noms longs (§DA-2)
