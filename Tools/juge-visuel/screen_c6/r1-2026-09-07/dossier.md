# Dossier du juge visuel — ㊱ L'horizon des possibles — r1 — 2026-09-07
...
- **Nom** : L'horizon des possibles (㊱, canon `écran neuf, sans id canon`) — contrôleur `HorizonScreenController`
...
- **Chemin joueur emprunté par la capture** : Plus → L'HORIZON DES POSSIBLES
- **États capturés** : PREMIER TOUR (r1) — aucune planche jugée avant. 4 capture(s), 2 campagne(s) : `03efb90` 06/09 14:56 · `fd0e21e` 06/09 20:53. Planches = blobs commités sur `main` du client (`fd0e21e` est le tip de la base de preuve du 06/09 soir) ; aucun correctif d'écran antérieur ⇒ pas de descendance à vérifier, pas de tour précédent.

## Référence (fait autorité : l'IMAGE)

| fichier (dans ce dossier) | rôle | taille px | facteur | largeur CSS ↔ largeur écran |
|---|---|---|---|---|
| `reference-1080x2102.png` | rendu du cadre nominal (`ecrans-brennar-6.html` #113) | 1080×2102 | ×3,6 | 300 CSS = 1080 px |
...
- **Source HTML/CSS** (aide de lecture, ne prime JAMAIS sur l'image) : `/home/erutheone/project/atelier3d-mafia/ecrans-brennar-6.html` (atelier `9fd7b6f` ;
  ...
  - #113 (l.5879) — L'horizon — ce qui s'ouvre et à quel prix  ⇐ **cadre NOMINAL, rendu en référence**
  - #114 (l.5882) — Le rabatteur a reculé
  - #115 (l.5885) — Prendre le second couteau
  - #116 (l.5888) — Sans les textes — l'écran tel qu'il s'affiche aujourd'hui
  - #117 (l.5891) — Rien à l'horizon
  - #118 (l.5894) — Ce qui manque encore
...
|---|---|---|---|---|
| `capture-1080x2400.png` | 1080×2400 — PRINCIPALE | ÉTAT VIDE, sous chrome (`VuePrincipaleCapturePlayModeTests`) — campagne `fd0e21e` (06/09 20:53) | 06/09 20:53 (`fd0e21e`) | `screen_c6_horizon_etat-vide_sous_chrome_1080x2400.png — sha256 e4b4a15e…` |
| `capture-ecran-seul-etat-vide-1080x2400.png` | 1080×2400 | ÉTAT VIDE, écran SEUL sans chrome (`HorizonScreenPlayModeTests`) — autre campagne — campagne `03efb90` (06/09 14:56) | 06/09 14:56 (`03efb90`) | `screen_c6_horizon_etat-vide_1080x2400.png — sha256 1b9a4d30…` |
| `capture-ecran-seul-1080x2400.png` | 1080×2400 | écran SEUL, état non déclaré par le nom (`HorizonScreenPlayModeTests`) — autre campagne ; à LIRE — campagne `03efb90` (06/09 14:56) | 06/09 14:56 (`03efb90`) | `screen_c6_1080x2400.png — sha256 19c810fd…` |
| `capture-ecran-seul-1080x1920.png` | 1080×1920 | idem, 16:9 — campagne `03efb90` (06/09 14:56) | 06/09 14:56 (`03efb90`) | `screen_c6_1080x1920.png — sha256 7a5b23cf…` |

- Client au moment des captures : `03efb90` (`main` du client, 06/09 14:56) / `fd0e21e` (`main` du client, 06/09 20:53) — une capture par campagne, voir la table ; ce dossier est préparé sur `28b9da5`. Une capture est une mesure DATÉE.
...
- **PRIORITÉ 1 (user)** : cet écran est fourni en ÉTAT VIDE. Juge d'abord le SENS de ce vide (voir la règle plus bas), puis la forme contre le cadre d'état homologue du groupe #113–118 (SOURCE seulement — dis lequel tu as choisi et pourquoi).
