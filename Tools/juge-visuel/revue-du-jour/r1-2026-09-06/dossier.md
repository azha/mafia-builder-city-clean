# Dossier du juge visuel — ⑯ La revue du jour — r1 — 2026-09-06
...
- **Nom** : La revue du jour (⑯, canon `screen_11`) — contrôleur `DailyReviewScreenController`
- **Ce qu'on vient y faire** : le matin au Verge d'Or : chaque homme qui a dévié de sa routine a posé un jeton de confiance sur le zinc ; on le lui rend (valider) ou on le garde (passer outre, en appui long), et on tamponne le registre des routines.
- **Chemin joueur emprunté par la capture** : Plus → LA REVUE DU JOUR (monté en surimpression par le test de planche, compte de démo).
- **États capturés** : un seul : le compte de démo au 2026-09-04 (des signalements existent, ou « personne au comptoir »).

## Référence (fait autorité : l'IMAGE)

| fichier (dans ce dossier) | rôle | taille px | facteur | largeur CSS ↔ largeur écran |
|---|---|---|---|---|
| `reference-1080x2102.png` | rendu du cadre nominal (série 4 #0 « trois jetons sur le zinc », canon ratifié) | 1080×2102 | ×3,6 | 300 CSS = 1080 px |
...
- **Source HTML/CSS** (aide de lecture, ne prime JAMAIS sur l'image) : `/home/erutheone/project/atelier3d-mafia/ecrans-brennar-4.html` (atelier `eb1eb55` ;
  ...
  - #1 (l.581) — Revue du jour — personne au comptoir
  - #2 (l.592) — Revue du jour — après vos verdicts
  - #3 (l.608) — Revue du jour — avec les lots back L1 + L2
...
- Série 6, mêmes cadres en matière « registre » : `/home/erutheone/project/atelier3d-mafia/ecrans-brennar-6.html` #0-3 (l.697-727).
...
|---|---|---|---|---|
| `capture-1080x2400.png` | 1080×2400 | compte de démo, surimpression | 2026-09-04 11:22 | `PlancheEcransManquantsCapturePlayModeTests (`planche_la_revue_du_jour_1080x2400.png`)` |
...
- ⚠️ La seconde capture n'est PAS comparable à la première (deux variables : le compte et le bundle i18n). Sers-t'en seulement pour voir la FORME d'une liste garnie si la première est vide — jamais pour un delta chiffré entre les deux.
