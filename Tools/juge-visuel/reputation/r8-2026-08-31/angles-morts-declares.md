...
| A3 | **Les espacements à l'écran** — partiellement couvert par `B3S4` (le bloc élastique absorbe la hauteur ajoutée ET son contenu garde la même hauteur aux deux résolutions ; contrôle négatif exécuté). ⚠️ Je l'avais déclaré FERMÉ au tour précédent : il ne l'était pas, la garde mesurait un total et restait verte pendant que le mou partait dans la carte du portrait. Une garde ne ferme que ce qu'elle MESURE | les constantes sont vérifiées contre la maquette (42 concordances), leur EFFET ne l'est pas. Un `LayoutGroup` mal câblé rendrait des valeurs justes invisibles. | le juge visuel |
...
| A5 | **Les états `derive` / `gages` / `vide`** — ⚠️ moitié RENDU fermée par la garde `B3S5` (les trois états rendent trois verdicts, trois couleurs et trois sur-titres distincts ; exercée, verte). La moitié CONTRAT reste ENTIÈRE : le corps est fabriqué par le test, donc rien ne prouve que le serveur émette ces valeurs ni sous cette forme. Et aucune IMAGE de ces états n'est capturée, donc rien n'en est vérifié visuellement | les tests exercent l'état d'un compte frais (`indeterminate`, 0 règle). `drifting`, `hostile`, `wary` et la liste pleine ne sont atteints par AUCUN test — leur code existe et n'est jamais exécuté. | un scénario qui déclare 4 règles et provoque une violation, ou un seed |
...
Elle vérifie deux hauteurs — celle du bloc élastique et celle de son contenu. Elle ne dit rien de :
la répartition à l'intérieur du contenu, les espacements entre blocs, les paddings, ni aucun rapport
horizontal. Un défaut de mise en page hors de ces deux mesures la laisserait verte, comme il l'a
déjà fait une fois.
