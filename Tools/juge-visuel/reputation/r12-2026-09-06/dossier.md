# Dossier du juge visuel — ㊲ La réputation (« le miroir ») — r12 — 2026-09-06
...
- **États capturés** : celui du compte post-campagne — VIERGE (cadre #120) ou GARNI (cadre #119) : ton inventaire le dira, c'est LE témoin à choisir. Cinq planches du MÊME commit `fd0e21e` (merge de `main`, 20:53 — planches et correctifs enfin dans le MÊME arbre : `pilote-F` et `correcteur/ecrans` 89231b3 en sont ancêtres) : sous chrome 2400 (PRINCIPALE) et 1920, écran seul 2400 et 1920, et la **paire T / T+1 s** à 1920 (écran seul).
...
- **les tours r1→r11 existent** (`Tools/juge-visuel/reputation/`) — pas fournis ; seule `grandeurs-r9-r11.md` t'est donnée ;
- aucun corps réel comparable (base post-campagne sans corps) ; le journal du run ;
