# Dossier du juge visuel — ㊲ La réputation (« le miroir ») — r11 — 2026-09-06
...
- **Chemin joueur emprunté par la capture** : onglet PLUS → « LA RÉPUTATION » — **captures SOUS le chrome du shell** (bandeau + dock réels), les deux résolutions.
- **États capturés** : un seul : le compte gelé — VIERGE (cadre #120) ou GARNI (cadre #119) : ton inventaire le dira, c'est LE témoin à choisir.
...
|---|---|---|---|---|
| `capture-1080x1920.png` | 1080×1920 (16:9) | compte gelé, SOUS chrome | 2026-09-06 13:33 (`3b0ffae`, `correcteur/ecrans`) | `VuePrincipaleCapturePlayModeTests (`screen_b3_reputation_sous_chrome_1080x1920.png`, sha256 `563e129f…`)` |
| `capture-1080x2400.png` | 1080×2400 (20:9, cible téléphone) | compte gelé, SOUS chrome | 2026-09-06 13:33 (`3b0ffae`) | `idem (`screen_b3_reputation_sous_chrome_1080x2400.png`, `a49fc1d0…`)` |

- Client au moment des captures : `76ee3cc` (`main`, 2026-09-04 11:23 — « les QUINZE planches reprises sur le
  MÊME bundle (674 clés) ») ; ce dossier est préparé sur `cc51b38`. Une capture est une mesure DATÉE.
- Compte photographié : `demo_capture@example.test`, compte GELÉ (empreinte de référence, minute 72 118), régime=env rapporté pour ces planches, `[CHROME-CAPITALE]` 19 px sur les deux.
...
- **Identité** : régime=env, compte gelé, `[CHROME-CAPITALE]` 19 px sur les deux (rapporté par f2 ; journal non joint).
- **Aucune paire T/T+1 s** dans ce dossier : la stabilité n'est PAS vérifiable ici — écris-le en non-vérifié (le journal du run est à demander au correcteur).
- **Ce que le lot de correctifs DÉCLARE (`542aff9` → `4f8e1ae` → `6a008c3` → `17c7036` → `3b0ffae`) — à vérifier, pas à croire** : (a) **ancrage du cadre EN BAS** (référence : filet y 452..2078 sur 2102, 24 px sous lui ; l'arithmétique « 122 + 462 = 584 » n'était juste qu'à la résolution de la référence) ; (b) **F3 axe du buste** fermé par une translation rigide de −12,93 u (le parent mesurait 435,5 au lieu de 409,6) ; (c) **F6 interligne** posé partout (pas de ligne 1,400 contre 1,164) et **F8 bordure** comptée (8/9) ; (d) **F2 calotte — la FORME a changé** : une ellipse ne pouvait porter aucun des quatre nombres ⇒ rectangle arrondi r = 11 sur 16 de haut, largeur pleine à la jonction, creux du front en occlusion par trois points (18;26) (31;21) (44;26) — **le correcteur n'a remesuré AUCUN de tes quatre nombres** (largeur calotte ÷ tête au max et à la jonction, hauteur d'attache, épaisseur à 15 % du visage, courbure du bord bas) : c'est à toi ; (e) **F4** trait centré sur le chemin (remplissage du visage prédit 23,25 u contre 22,97 réf / 25,22 jeu) ; (f) **F11** bouche 9 → 10,7 u de longueur (la hauteur d'encre 2,19/2,55 vient des ellipses d'occlusion, non touchée).
- **F1 (tiers bas du panneau vide) : NON traité, délibérément** — sa décomposition est aux 4/5 une conséquence de F6/F8 et le panneau est élastique : **remesure-le**, ne le cherche pas comme correctif.
- **F5 (halos cyan / lueur interne des chiffres) : OUVERT**, pas fait — remesure tel quel.
- **Gouttière : tranche-la sur cette paire** (captures sous chrome) — le r10 sans chrome ne le pouvait pas : rien du cadre sous le bandeau (143 px) ni sous le dock ; où commence le cadre par rapport au bas du bandeau, où finit-il par rapport au dock, aux deux résolutions.
- **Convention de bord** : déclare-la pour toute épaisseur de trait (nominale à mi-alpha, ou cœur opaque au seuil).
- Fichiers touchés depuis le r10 (`0da8895` → `3b0ffae`, sans stats) : `Operational/Reputation/ReputationPortrait.cs`, `ReputationScreenController.cs`, `Tests/PlayMode/ReputationScreenPlayModeTests.cs`.
- **Grandeurs du tour précédent** : `grandeurs-r10.md` (paire SANS chrome de `0da8895`) — valeurs sans verdict, pour la colonne `critère`. Rien d'autre des tours r1→r10 n'est fourni.
...
| la ligne de balayage teal au tiers haut du panneau est PRÉSENTE, fixe | trait d'identité (e) de la maquette (`.elast::after`), figée dans le rendu ratifié | absente, animée, ou ailleurs que dans le tiers haut |
...
- **dix tours (r1 → r10) existent dans `Tools/juge-visuel/reputation/`** — pas fournis ; seule `grandeurs-r10.md` t'est donnée ;
- le journal du run et une paire T/T+1 s ;
