...
- Client au moment des captures : `d5ddc40` (`correcteur/ecrans`, 07/09 04:27), arbre de rendu = le commit des planches à 1 commit près (le journal ne l'imprime pas) ; ce dossier est préparé sur `5ff189d`. Une capture est une mesure DATÉE.
...
- **Ce que le lot DÉCLARE (`fd0e21e` → `d5ddc40`) — à vérifier, pas à croire** :
  ...
  - **`5e61061` — district : les 7 types de bâtiment AVEC art (laboratoire, planque, serre, coffre, point de vente…) n'avaient NI sprite NI libellé** (une condition datée : le libellé n'était posé que pour les types au sprite de repli ; l'image ayant été éteinte par l'arbitrage du 22/08, ces 7 types étaient « le même bâtiment anonyme du fond »). Déclaré corrigé : le libellé est posé dès que le sprite ne discrimine pas. ⇒ Mesure : chaque cellule de bâtiment du joueur porte-t-elle un libellé de type lisible, et pour les 7 types avec art comme pour les 6 sans ? (`icon_archetype` / glyphe : à lire sur l'image.)
  - **`5bf0ba8` — ① M3 du r8 (voile du bandeau, +29/255 trop clair, alpha non converti pour le linéaire) : déclaré PROUVÉ EN COMPOSITION** par une garde (`DA10`) — l'alpha recopié rate de 36,04/255 et 41,24/255 sur le pire des 7 fonds, l'alpha ajusté de 3,83 et 4,33. ⇒ Remesure le pixel résultant du voile sur l'art réel (méthode r8 : même fond, prédiction sRGB vs linéaire).
  - **`7148848` + `5bf0ba8` — ① M2 du r8 (arcs 1,6–1,8× trop épais) : MESURÉ, PAS CORRIGÉ** — le rastériseur rend 3,375 px pour `t = 5` (pente 1,000) et le juge en lisait 4,20 : l'excédent est un FACTEUR 1,244 HORS du rastériseur, propre à l'épaisseur seule (boîtier 0,985, rayon 1,023). Aucun correctif déclaré ⇒ attends-toi à le remesurer identique ; s'il l'est, c'est un écart RÉCURRENT (r7, r8, r9) = fond, pas un correctif de plus.
  - Rien d'autre n'est déclaré corrigé sur ① (halo du cerclage M1, volute droite, aiguille, pivot, éclairage du cadran, ruban d'alerte, flou de la plaque… : NON déclarés) — remesure-les, et marque RÉCURRENT ce qui l'est.
