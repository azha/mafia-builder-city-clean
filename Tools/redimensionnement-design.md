# Lot « redimensionnement » — **v17** — le client survit à un changement de taille en cours de vie
...
> **v15 NOT_APPROVED** (4B/6I/6m) · **v16 NOT_APPROVED** (4B/5I/3m). *(la v14 n'a pas été routée séparément — son delta est entré dans la v15.)*
...
> peut pas mentir par omission. ⇒ **Le compte se lit SUR la liste ci-dessus — aucun total n'est écrit ici.** *(B3 de la v16 : ma « correction » de I4 avait écrit un total de plus, faux d'exactement 1 — la v14, exclue par la parenthèse voisine. **Cinquième péremption du même total, dans le correctif de la quatrième.** Le bloc interdisait déjà le geste onze lignes plus bas : « le correctif d'un total recopié n'est pas d'en écrire un autre ». Si un total est indispensable un jour, il sera une SORTIE D'INSTRUMENT, jamais une prose.)* Rapports hors dépôt : `/tmp/revue-redimensionnement-design{,-v2,-v3,-v4}.md`.
...
3. ⛔⛔ **B1 de la v5 — L'ANTI-VACUITÉ DISJONCTIVE EXCLUAIT LE MONDE QU'ELLE DEVAIT COUVRIR.**
   ...
   - **S1 — RÉTABLI en v11, sur un couple PAYSAGE.** Tue **c2** (le retour anticipé de `ZoomTo`).
   ...
   *Une disjonction laisse le scénario choisir lequel il satisfait ; deux scénarios ne le laissent pas.*
   ...
   |---|---|---|---|
   ⛔⛔⛔ **B3 de la v10 — SUPPRIMER S1 ÉTAIT UNE CAPITULATION, ET « INATTEIGNABLE » QUALIFIAIT
   MON CORPUS DE TEST, PAS LE MONDE.** La v10 a écrit : *« c2 exige `contain < 1` aux deux bouts ;
   une seule des QUATRE RÉSOLUTIONS l'a »* — et ces quatre résolutions sont un `[TestCase]` que **ce
   lot est déjà en train d'étendre** (⑮). Pendant ce temps le §0, **à 0 delta depuis des versions**,
   établit le monde de production :
```
   androidResizeableActivity: 1        ⇒ multi-fenêtres AUTORISÉ
   androidDefaultWindowWidth : 1920
   androidDefaultWindowHeight: 1080    ⇒ la fenêtre par défaut est PAYSAGE

   fond 1080×1920 :  1920×1080 → contain 0,5625 < 1      ⇐ un pan EXISTE
                     1920×600  → contain 0,3125 < 1      ⇐ après glissement du séparateur
```
   ⇒ **c2 mord un joueur réel, sur le geste même que ce lot existe pour supporter** : il glisse le
   séparateur, et son cadrage disparaît en silence. **J'ai supprimé le scénario qui teste un défaut
   de PRODUCTION parce que mon corpus de TEST ne contenait pas son monde.**
   ⇒ ***« Inatteignable » est une propriété du corpus ; « n'arrive pas » est une propriété du monde.
   Les confondre transforme un trou de couverture en démonstration d'impossibilité.***

   ⇒ **Et le correctif juste était écrit dans le bloc qui a servi à supprimer.** La v10 y démontrait :
   *« un pan à restaurer doit exister là où il est SAUVEGARDÉ, donc au DÉPART »*. **C'est la garde de
   capacité correcte** — asserter `PanPosition != Vector2.zero` **AVANT** la reconstruction. Elle est
   verte pour c2 comme pour le correctif juste (les deux ont un pan au départ), donc elle **ne
   certifie rien** ; et l'assertion, elle, **discrimine** : c2 rend `initialFocus`, le correctif rend
   le pan sauvegardé. *Le document avait trouvé la réparation et s'en est servi pour justifier une
   suppression.*

   | | condition de monde | couple | garde de capacité |
   |---|---|---|---|
   | **S1** | `contain < 1` **AUX DEUX BOUTS** — le monde du multi-fenêtres paysage, celui du §0 | **1920×1080 → 1280×720** ⛔⛔ **B2 de la v11 — CES DEUX POINTS NE SONT PAS RENDUS PAR LE HARNAIS, ET MA JUSTIFICATION ÉTAIT UNE FORME E.** J'écrivais « `1280×720` est déjà dans le `[TestCase]` » : ce `[TestCase]` se déclare **PUR dans son propre en-tête** (`DistrictMapNavigationPlayModeTests.cs:497-499` — *« pures — sans Canvas ni Screen »*). **Une résolution passée en ARGUMENT DE FONCTION n'est pas une résolution que le harnais REND.** Et `1920×1080` : **0 fichier sur 146** sous `Assets/`. ⇒ **Livrable ㉙** | ⛔ **B2 de la v13 : `PanPosition != Vector2.zero` NE DISCRIMINE PAS.** Le discriminant écrit au §3 est
   *« c2 rend `initialFocus`, le correctif rend le pan sauvegardé »* ⇒ la propriété est
   `pan_sauvegardé ≠ ClampPan(−initialFocus)`. Les deux ne coïncident **que là où `ClampPan` annule
   `initialFocus`** — ce que la ligne 28 mesure **en PORTRAIT**, alors que **S1 est PAYSAGE aux deux
   bouts**. Mesuré : sur tout `|φ| ≤ 0,2188` la garde est **VERTE** et l'assertion **VERTE sous c2**.
   *La garde certifie le défaut.* ⇒ **GARDE DE DELTA** : mémoriser `p0 = PanPosition` juste après le
   `Configure` de départ, puis asserter **`PanPosition != p0`** avant la reconstruction — le monde
   dégénéré devient **inexprimable** ⇒ **ET** l'atteignable d'arrivée **contient** celui du départ,
   par axe (㉘), après conversion (㉚) |
   ...
   ⛔⛔ **B2 de la v12 — `ClampPan` BORNE DEUX AXES INDÉPENDAMMENT, ET J'AI PUBLIÉ UN SEUL.**
   `DistrictMapNavigation.cs:218-220` clampe `x` et `y` **séparément** ; la grandeur assertée (règle 2)
   est un `Vector2` ; et ma table disait « fraction de la **hauteur** du fond ». ⇒ **L'instrument que
   R2 consomme ne mesurait que Y.** Contre-exemple mesuré : `1080×1920 → 1920×1080` à ×2 satisfait la
   condition écrite (longueurs de `ZoomLevels` différentes) **et** le contenant en Y (0,2500 ⊂ 0,3594),
   **en violant X** (0,2500 ⊄ 0,0556) ⇒ un joueur pané jusqu'à la borne en X n'est pas restaurable, et
   **l'assertion rougit sur le correctif juste** — le mode d'échec que la v12 existe pour fermer, un
   ...
   **ATTEIGNABLE NORMALISÉ, PAR AXE** — ⛔ **B3 de la v13 : mon libellé était FAUX D'UN FACTEUR `s`.**
   Il disait « fraction de la dimension du **fond** » ; le diviseur réel est celui que **définit la rangée `BORNE:DEF`** *(récit de la faute — la valeur du diviseur n'est PAS redite ici, sinon ce paragraphe redeviendrait un producteur)*. Mesuré à `1080×1920` ×2 : borne locale `[640,00 ; 1137,78]` ⇒ avec le mauvais diviseur **0,5000**,
   avec celui que définit la rangée `BORNE:DEF` **0,2500** — et c'est la seconde que la table publie.
   *(La valeur du bon diviseur n'est PAS écrite ici : le récit d'une faute d'unité n'exige que la
   faute, pas la définition — c'est ce que je croyais irréductible et qui ne l'était pas.)* ⚠️ **À ×1 les deux lectures coïncident**, donc l'erreur est invisible exactement là où on la
   ...
   référence — est précisément l'aveugle. *(Même famille que la plaque de verre qui tombait juste
   sans conversion sRGB/linéaire et semblait innocenter le modèle : un modèle faux peut être exact
   là où les grandeurs se confondent.)*
...
```
   ...
   1280×720        0,0000    0,3125    0,2037    0,4062
   1080×1920       0,0000    0,0000    0,2500    0,2500
   1440×3200       0,0000    0,0000    0,1667    0,0833

   S1  1920×1080 → 1280×720  à ×1 :  X 0,0000 ⊆ 0,0000 · Y 0,2188 ⊆ 0,3125   ✅ sur LES DEUX AXES
   S2  1440×3200 → 1080×1920 à ×2 :  X 0,1667 ⊆ 0,2500 · Y 0,0833 ⊆ 0,2500   ✅ sur LES DEUX AXES
```
   ⇒ **La propriété est une INCLUSION DE BOÎTES**, jamais un scalaire :
   `|p_départ|ₓ ≤ A_arrivée,ₓ` **ET** `|p_départ|ᵧ ≤ A_arrivée,ᵧ`.
...
- **(b) CADRAGE** — ⛔⛔ **B2 de la v6 : CE PARAGRAPHE ÉTAIT BYTE-IDENTIQUE À LA v5 ET PRESCRIVAIT
  ...
  **la seule occurrence PRESCRIPTIVE** du document — les autres sont la démonstration que la
  ...
  portant sa **garde de capacité AU DÉPART et sa borne telle que définie à la ligne 28 du plancher** *(I4 de la v11 : ce §,
  ...
  **garde DISJONCTIVE de la v5** — les quatre sont réfutés au §3. *(I1 de la v13 : écrire « la garde de non-défaultitude » collisionnait avec la garde de capacité que ⑫ PRESCRIT, et un R2 lisant ce § conservativement aurait supprimé la seule anti-vacuité de S1/S2. Nommer le construct par ce qu'il est, jamais par la propriété qu'il visait.)*
...
| 8 | ⚠️ **c2 — RETIRÉ des déduits en v11 : il n'était pas inatteignable, il était NON TESTÉ.** Le monde existe en production (multi-fenêtres paysage, §0) ; c'est mon CORPUS DE TEST qui ne le contenait pas. ⇒ **c2 est de nouveau couvert par S1**, couple `1920×1080 → 1280×720`. **Le détecteur ㉗ est SUPPRIMÉ** : il observait « combien de résolutions MON corpus contient », donc il était structurellement muet sur la question qui décide | — |
...
| 7 | §3 | S1/S2 avec conditions de monde, gardes de capacité **au départ**, et **la borne définie à la ligne 28** *(citation)* — *plus de condition de donnée : elle est inerte, cf. ligne 28* | R2 ⑫ |
...
| 28 | §3 | ⛔⛔ **DÉFINITION UNIQUE DE LA BORNE — tout autre endroit du document la CITE, aucun ne la REDIT.** *(v15 : quatre tours de suite, un correctif de côté a laissé le défaut de l'autre côté — arrivée/sauvegarde, consommateur/producteur, 2D/1D. Mesuré : **5 exemplaires, 0 citation**, chacun périssable indépendamment. Le correctif qui ferme la classe n'est pas un côté de plus, c'est **UN SEUL PRODUCTEUR** : une redite peut se périmer, une citation ne le peut pas. Même remède que le socle pour les types homonymes — ré-exporter depuis une source unique plutôt que corriger les deux.)* ⇒ **ET CE N'EST PAS QU'UNE CONVENTION : `Tools/borne-producteur-unique.py` l'ASSERTE.** Réduire cinq exemplaires à un ne ferme pas la classe — rien n'empêcherait un sixième, écrit par le geste le plus naturel du monde : quelqu'un qui trouve la citation indirecte et « clarifie » en recopiant. ⛔⛔ **ET MA PREMIÈRE VERSION DE CE DÉTECTEUR ÉTAIT FAUSSE — je l'ai annoncée comme fermant la classe, elle attrapait UNE formulation sur SIX.** Testée par la revue ⊥ : coupure de ligne, « en X et en Y », formule directe, accents NFD, ordre inversé — **cinq passent**, et **deux producteurs vivaient déjà dans ce §3** (l'un coupé par un retour à la ligne, l'autre à 1187 caractères). ★ Mon contrôle négatif n'avait testé que la tournure **identique** : il prouvait que le motif se reconnaît lui-même, jamais qu'il attrape la classe. *Une garde sur la TOURNURE présentée comme une garde sur la PROPRIÉTÉ.* ⇒ **Refait sur une ANCRE EXPLICITE** (un marqueur HTML nommé `BORNE:DEF`, dont le littéral n'est écrit qu'À SA POSITION — *l'écrire ici en le citant l'a compté comme une seconde définition, et mon propre détecteur a rougi sur mon propre document : le piège de citation du socle, appliqué à un marqueur*) que la paraphrase ne peut pas produire par accident, avec ses trois contrôles exécutés : marqueur présent ⇒ 0 · marqueur dupliqué ⇒ 1 · marqueur absent ⇒ 1. Et l'instrument **déclare** qu'il ne sait pas dire si les énoncés d'unité restants citent ou redisent : il décide sur le MARQUEUR, pas sur les mots. <!-- BORNE:DEF --> **LA BORNE** = l'atteignable **PAR AXE (X et Y)**, aux deux bouts de chaque scénario, publiée en valeurs, **en fraction de la dimension de CONTENU à l'échelle du palier** (`p/(fond·s)`) *(remplace la « condition de DONNÉE » de la v11 : **㉘ était INERTE** — `ClampPan` annule `initialFocus` à ×1 dans les deux formats portrait, donc un barycentre décentré ne produit **aucun** observable ; et `initialFocus` est une **variable locale sans accesseur**, donc non assertable sans changement de production)* | R1 **㉘** |
...
| 26 | §8 | **détecteur de c3** — `ZoomLevels[Length-1] == 3f` sur **toutes les résolutions que le harnais REND** *(I3 de la v13 : « aux 4 résolutions » était un compte figé, et ㉙ en ajoute un 5ᵉ — un détecteur de péremption à population gelée sous-couvre en silence)* | R1 **㉖** |
| 13 | §5 | zone sûre à valeurs distinctes via le seam | R3 ⑳ |
| 14 | §6 | (a) et (b) dans deux scénarios distincts | R2 ⑪ |
| 15 | §6.1 | seconde largeur de capture — **R1**, car S2 (chunk R2) en dépend et R3 court après | R1 ⑮ |
...
jamais recopié sa sortie** : la phrase était verbatim celle de la v9. ⇒ ***Un instrument réparé
dont on ne recopie pas la sortie vaut exactement l'instrument inerte qu'il remplace.***
⇒ **Correctif de FORME, pour que ce nombre ne puisse plus mentir** : cette ligne ne porte plus de
compte recopié. **Le compte se lit en exécutant la commande** :
```
python3 Tools/plancher-decoupage.py Tools/redimensionnement-design.md
```
Instrument commité : `Tools/plancher-decoupage.py` (m3 — *un chiffre dont l'instrument n'est pas dans le
