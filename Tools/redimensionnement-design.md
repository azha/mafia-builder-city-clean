...
> peut pas escamoter un de ses propres verdicts. Corrigé ici : les verdicts omis ont été rétablis dans la liste. **Aucun compte n'est écrit dans cette parenthèse** — B1 de la v17 : elle en portait un, deux lignes sous la phrase qui l'interdit, **inchangé depuis la v5**. Sixième péremption du même genre, et la règle voisine (« ajouter une version OBLIGE à ajouter sa ligne ») n'avait jamais été passée dessus.)*
>
> ★★★ **LE MOTIF, À CHAQUE TOUR DE LA LISTE CI-DESSUS, ET IL S'EST DÉPLACÉ VERS L'INTÉRIEUR** *(le compte se lit sur la liste — B1 de la v17 : « CINQ FOIS » énumérait quatre versions)* :
...
| 7 | ⛔ **c3 — un rang clampé atterrit sur un AUTRE rôle : INATTEIGNABLE** tant que `contain < 3`. **B1 de la v9 : elle annonçait « porté au §8 avec son détecteur » et ne l'avait fait NI ici NI dans une cellule — le livrable oublié des DEUX côtés, que la bijection ne peut pas voir.** | **détecteur ㉖** : asserter `ZoomLevels[Length-1] == 3f` sur **toutes les résolutions que le harnais REND**, la population étant celle que ㉙ et ⑮ fixent — jamais un compte figé ici *(trouvé le 2026-08-31 par ㉜, le balayage du corps : le §11 avait corrigé ce compte en v13 et **le corps le portait encore deux versions plus tard**. La jointure du socle — le texte que la correction devait rouvrir et n'a pas rouvert — et le premier défaut de cette classe trouvé par un INSTRUMENT et non par une revue.)* VERT aujourd'hui, **ROUGE dès qu'une résolution rend `contain ≥ 3`** — une épingle sur une VALEUR PRÉSENTE, jamais sur une absence |
