...
{
    // ㉓ LA VITRINE — capture du chemin de PRODUCTION, régime « full prod » du 2026-09-01.
    //
    ...
    [Category("PhotoVitrine")]
    public class LaVitrineCapturePlayModeTests
    {
        ...
        [UnityTest]
        public IEnumerator Capture_LaVitrine_1080x2400()
        {
            ...
            const string Chemin = "Assets/Screenshots/la_vitrine_1080x2400.png";
            ...
            shell.MonterLocataireEnSurimpression<ShopScreenController>();
            ShopScreenController vente = null;
            float montage = 0f;
            while (montage < 15f && vente == null)
            {
                vente = shell.ContentSlot.GetComponentInChildren<ShopScreenController>(true);
                montage += Time.deltaTime;
                yield return null;
            }
            Assert.IsNotNull(vente,
                "ShopScreenController non monté en surimpression — la capture montrerait une destination vide");
            ...
            float charge = 0f;
            while (charge < 20f && vente.Catalogue == null && !vente.EtatVide)
            {
            ...
            Debug.Log($"[VITRINE diag] chemin={chaine} actif={vente.gameObject.activeInHierarchy} " +
            ...
            GameObject camGo = new GameObject("CaptureVitrineCam");
            ...
            Debug.Log($"[CAPTURE] {Chemin} {Largeur}x{Hauteur} — {teintes.Count} teintes distinctes · " +
                      $"skus={(vente.Catalogue != null ? vente.Catalogue.Length : -1)} solde={vente.Solde} vide={vente.EtatVide}");
