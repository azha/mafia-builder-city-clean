...
namespace MafiaCleanCity.Shell.Tests
{
    ...
    [Category("PhotoDecision")]
    public class DecisionDetailCapturePlayModeTests
    {
        ...
        [UnityTest]
        public IEnumerator Capture_DecisionDetail_1080x2400()
        {
            ...
            const string Chemin = "Assets/Screenshots/decision_du_jour_1080x2400.png";
            ...
            float attenteBloc = 0f;
            while (attenteBloc < 15f && bloc == null)
            {
                bloc = shell.ContentSlot.GetComponentInChildren<HighestLeverageCardController>(true);
                attenteBloc += Time.deltaTime;
                yield return null;
            }
            Assert.IsNotNull(bloc, "le bloc HL de l'Accueil n'est pas monté — sans lui ⑤ n'a aucun déclencheur");
            bloc.DemanderDetail();
            DecisionDetailScreenController decision = null;
            float montage = 0f;
            while (montage < 15f && decision == null)
            {
                decision = shell.ContentSlot.GetComponentInChildren<DecisionDetailScreenController>(true);
                montage += Time.deltaTime;
                yield return null;
            }
            Assert.IsNotNull(decision,
                "DecisionDetailScreenController non monté sous l'onglet More — la capture montrerait une destination vide");
            ...
            float charge = 0f;
            while (charge < 20f && decision.CarteAffichee == null && !decision.EtatSabot)
            {
            ...
            Canvas canvas = decision.GetComponentInParent<Canvas>();
            ...
            Debug.Log($"[CAPTURE diag] canvas={canvas.name} mode={canvas.renderMode} " +
                      ...
                      $"decision.parent={(decision.transform.parent != null ? decision.transform.parent.name : "AUCUN")}");
            ...
            Debug.Log($"[CAPTURE] {Chemin} {Largeur}x{Hauteur} — {teintes.Count} teintes distinctes · " +
                      $"carte={(decision.CarteAffichee != null ? decision.CarteAffichee.card_id : "aucune")} sabot={decision.EtatSabot}");
            ...
            Transform parentDuDecision = decision.transform.parent;
            Assert.AreEqual(parentDuDecision.childCount - 1, decision.transform.GetSiblingIndex(),
                $"le locataire est le frère {decision.transform.GetSiblingIndex()} sur "
                + $"{parentDuDecision.childCount} — les suivants se dessinent PAR DESSUS et la "
                + "capture montrerait les écrans du dessous, à la bonne taille.");
            RectTransform locataireRt = (RectTransform)decision.transform;
